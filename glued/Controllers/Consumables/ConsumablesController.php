...
namespace Glued\Controllers\Consumables;
...
class ConsumablesController extends Controller
{
    ...
    public function consumablesGui($request, $response)
    {
        $costs_output = '';
        $sloupce = array("c_uid", "c_data->>'$.data.category' as type", "c_data->>'$.data.item_name' as name", "c_data->>'$.data.locations[*].stock' as stock");
        $this->container->db->orderBy("c_uid","asc");
        $items = $this->container->db->get('t_consumables_items', null, $sloupce);
        if (count($items) > 0) {
            foreach ($items as $data) {
                ...
                $costs_output .= '
                    <tr>
                        ...
                        <td>'.$data['name'].'</td>
                        <td>'.$stock.'</td>
                        <td><a href="'.$this->container->router->pathFor('consumables.editform', ['id' => $data['c_uid']]).'">edit</a></td>
                    </tr>
                ';
            }
        }
    ...
    public function addStockForm($request, $response)
    {
        ...
        $json_schema_output = file_get_contents(__DIR__.'/V1/jsonschemas/new_consumables_form.json');
    ...
    $.ajax({
      url: "https://'.$this->container['settings']['glued']['hostname'].$this->container->router->pathFor('consumables.api.new').'",
      dataType: "text",
      type: "POST",
      ...
      error: function(xhr, status, err) {
        ...
        return $this->container->view->render($response, 'consumables/addstock.twig', array(
    ...
    public function editStockForm($request, $response, $args)
    {
        ...
        if ($this->container->permissions->have_action_on_object('t_consumables_items', $args['id'], 'write')) {
            ...
            $this->container->db->where("c_inherit_table", "t_consumables_items");
    ...
    public function uploaderSave($request, $response, $args)
    {
        ...
        if (isset($_POST['upload_formularem'])) {
            return $response->withRedirect($this->container->router->pathFor('consumables.editform', ['id' => $item_id]));
        }
        else {
            // vratime prosty text
