...
class ConsumablesControllerApiV1 extends Controller
{
    ...
    public function insertStockApi($request, $response)
    {
        ...
        $insert = $this->container->db->insert('t_consumables_items', $data);
        
        if ($insert) {
            $editacni_adresa = 'https://'.$this->container['settings']['glued']['hostname'].$this->container->router->pathFor('consumables.editform', ['id' => $insert]);
        }
        else {
            $editacni_adresa = 'https://'.$this->container['settings']['glued']['hostname'].$this->container->router->pathFor('consumables.addform');
        }
    ...
    public function editStockApi($request, $response, $args)
    {
        ...
        $update = $this->container->db->update('t_consumables_items', Array ( 'c_data' => $senddata ));
